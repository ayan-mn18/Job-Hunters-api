...
| **YC Work at a Startup** | `workatastartup.com` returns **406** to any non-browser client, whatever user agent it sends. `robots.txt` is fully permissive (`Disallow:` empty). Browsing needs no account; applying does. | Relative | **Solved, through Firecrawl.** The old direct-fetch adapter was silently returning nothing for exactly this reason. It is now a site skill (`src/skills/workatastartup/`) that reads through Firecrawl and applies through a signed-in hosted browser. Greenhouse/Lever/Ashby remain the cheaper path to the same companies where they publish there. |
